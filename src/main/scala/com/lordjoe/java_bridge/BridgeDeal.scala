...
object Position {
  ...
  def fromInt(i: Integer) : Position = positions(i)
...
object Hand {
  ...
  def fromCardJavaList(p: Position,cards: java.util.List[Card])  : Hand = {
    Hand(p,null,cards.asScala.toList)
  }
...
//  val clubs = ofSuit(Suit.Spade)
 ...
 // so Java can access list
  def getCard(i: Integer) : Card = cards(i)
...
object Deal {
   def fromJavaHandList(hands: java.util.List[Hand])  : Deal = {
     new Deal(Position.random,Vulnerability.random,hands.asScala.toList)
  }
